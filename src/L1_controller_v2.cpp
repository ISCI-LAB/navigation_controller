...
class L1Controller
{
	...
	geometry_msgs::Quaternion odom_goal_ori;
        ...
        int controller_freq;
        bool foundForwardPt, goal_received, goal_reached, goal_reached_ori;
	int smooth_counter;
...
L1Controller::L1Controller()
{
    ...
    path_sub = n_.subscribe("/move_base/GlobalPlanner/make_plan", 1, &L1Controller::pathCB, this);
    ...
    pub_ = n_.advertise<geometry_msgs::Twist>("/navigation_velocity_smoother/raw_cmd_vel", 1);
    result_ = n_.advertise<move_base_msgs::MoveBaseActionResult>("move_base/result_", 10); //// "move_base/result"

    //Timer
    timer1 = n_.createTimer(ros::Duration((0.1)/controller_freq), &L1Controller::controlLoopCB, this); // Duration(0.05) -> 20Hz
...
void L1Controller::goalReachingCB(const ros::TimerEvent&)
{
	...
	if(goal_reached == true && goal_reached_ori == true){
	    goal_reached = false;
	    goal_reached_ori = false;
            goal_received = false;
            ROS_INFO("Goal Reached !");
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)
{
    ...
    if(goal_received)
    {
		...
		if(cmd_vel.angular.z >= 30.0 / 180.0 * PI){
			cmd_vel.angular.z = 30.0 / 180.0 * PI;
			cmd_vel.linear.x = 0;
	    	}
	    	else if(cmd_vel.angular.z <= -30.0 / 180.0 * PI){
			cmd_vel.angular.z = -30.0 / 180.0 * PI;
			cmd_vel.linear.x = 0;
            	}
	  
        }
	else if(!goal_reached_ori){
		if(smooth_counter > 0){
			smooth_counter--;
		}
		else{		
