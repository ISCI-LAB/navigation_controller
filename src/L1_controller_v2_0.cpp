...
L1Controller::L1Controller()
{
    ...
    goal_sub = n_.subscribe("/move_base_simple/goal_", 1, &L1Controller::goalCB, this);
    marker_pub = n_.advertise<visualization_msgs::Marker>("car_path", 10);
    pub_ = n_.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/safety_controller_", 1);
    //******************1025*****************
    pub_pose = n_.advertise<geometry_msgs::Pose>("pose", 1);
    //***************************************
    ...
    //1209
    goal_pose = n_.advertise<geometry_msgs::PoseStamped>("goal", 1);
    //1209
...
void L1Controller::obstacle_scan(){
	...
	if(odom_goal_pos.z==1&&costmap_received==true){
		...
		if(obs_num>30 &&obs_pub_flag==false){
			//goalID++;
			//if(odom_goal_pos)
			obs_pub_flag=true;
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)
{
    ...
    //ROS_ERROR("%.2f %.2f",cmd_vel.linear.x,cmd_vel.angular.z);
